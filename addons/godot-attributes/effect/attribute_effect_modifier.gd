...
## Conditions that must be met for this modifier to modify an [ActiveAttributeEffect].
...
## [param active], returning true if there are no conditions or all conditions are met,
## false if not.
func should_modify(attribute: Attribute, active: ActiveAttributeEffect) -> bool:
	for condition: AttributeEffectCondition in conditions:
		if !condition.meets_condition(attribute, active):
			return false
	return true


## Must be implemented to modify the [param value] based on the context of [param attribute]
## and [param active]. Must return the modified value, or can return the [param value]
## the leave it "unmodified".
## [br]NOTE: The context parameters, [param attribute] and [param active] should NOT be modified here.
